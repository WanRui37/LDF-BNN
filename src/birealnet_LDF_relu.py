...
class Attention(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, drop_rate=0.1, gamma=1e-6, groups=1):
        ...
        self.se = SqueezeAndExpand(planes, planes, attention_mode="relu")
        self.scale = nn.Parameter(torch.ones(1, planes, 1, 1) * 0.5)

    def forward(self, input, se_in):
        if self.training:
...
class FFN_3x3(nn.Module):
    def __init__(self, inplanes, planes, stride=1, downsample=None, drop_rate=0.1, gamma=1e-8, groups=1):
        ...
        self.se = SqueezeAndExpand(inplanes, planes, attention_mode="relu")
        self.scale = nn.Parameter(torch.ones(1, planes, 1, 1) * 0.5)
...
class BasicBlock(nn.Module):
    ...
    def forward(self, input, se_in):
        x, se_out = self.Attention(input, se_in)
        y, se_out = self.FFN(x, se_out)

        return y, se_out
      

class LDFBNN18(nn.Module):
    def __init__(self, num_classes=1000, block = BasicBlock, layers = [2, 2, 2, 2]):
        super(LDFBNN18, self).__init__()
        ...
        self.se_out = nn.Parameter(torch.ones(1,64,1,1), requires_grad=True)
        ...
        self.layer0.append(block(self.inplanes, planes, stride, downsample, drop_rate = 0))
        self.inplanes = planes * block.expansion
        for i in range(1, layers[0]):
            self.layer0.append(block(self.inplanes, planes, drop_rate = 0))

        planes = int(width*128)
        stride = 2
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
        ...
        self.layer1.append(block(self.inplanes, planes, stride, downsample, drop_rate = 0))
        self.inplanes = planes * block.expansion
        for i in range(1, layers[1]):
            self.layer1.append(block(self.inplanes, planes, drop_rate = 0))

        planes = int(width*256)
        stride = 2
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
        ...
        self.layer2.append(block(self.inplanes, planes, stride, downsample, drop_rate = 0))
        self.inplanes = planes * block.expansion
        for i in range(1, layers[2]):
            self.layer2.append(block(self.inplanes, planes, drop_rate = 0))

        planes = int(width*512)
        stride = 2
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
        ...
        self.layer3.append(block(self.inplanes, planes, stride, downsample, drop_rate = 0))
        self.inplanes = planes * block.expansion
        for i in range(1, layers[3]):
            self.layer3.append(block(self.inplanes, planes, drop_rate = 0))
    ...
    def forward(self, x):
        ...
        se_out = self.attention(self.se_out)
        
        for i, block in enumerate(self.layer0):
            x, se_out = block(x, se_out)
        se_out = torch.cat((se_out, se_out), dim=1)
...
if __name__ == "__main__":
    ...
    model = LDFBNN18(num_classes = 1000).cuda()
