...
class BasicBlock(nn.Module):
    ...
    def forward(self, input, se_in):
        ...
        if self.inplanes == self.planes:
          ...
          y_1, se_out_1 = self.FFN_1(x, se_out)
          y_2, se_out_2 = self.FFN_2(x, se_out)
          y = torch.cat((y_1, y_2), dim = 1)
          se_out = torch.cat((se_out_1, se_out_2), dim = 1)
        return y, se_out
...
class LDFBNN(nn.Module):
    def __init__(self, num_classes=1000, size = "tiny", ELM_Attention = True, Infor_Recoupling = True):
        ...
        self.se_out = nn.Parameter(torch.ones(1,stage_out_channel[0],1,1), requires_grad=True)
        ...
        self.len = len(stage_out_channel)
    ...
    def forward(self, x):
        ...
        for i, block in enumerate(self.feature[1:self.len]):
...
def ldfbnn_tiny(**kwargs):
    model = LDFBNN(num_classes = 1000, size = "tiny", ELM_Attention = True, Infor_Recoupling = True)
    return model

def ldfbnn_small(**kwargs):
    model = LDFBNN(num_classes = 1000, size = "small", ELM_Attention = True, Infor_Recoupling = True)
    return model

if __name__ == "__main__":
    model = nn.DataParallel(LDFBNN(num_classes=1000, size="large")).cpu()
