...
from bnext_LDF import LDFBNN, BasicBlock
from birealnet_LDF import LDFBNN18

TRAIN_ONCE = 0
parser = argparse.ArgumentParser("bnext")
parser.add_argument('--model', type=str, default="ldfbnn_tiny",
                    help="student model")
...
def adjust_temperature(model, epoch):
  ...
  if args.model == "ldfbnn18":
      from birealnet_LDF import HardSign, HardBinaryConv
  else:
      from bnext_LDF import HardSign, HardBinaryConv
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.model == "ldfbnn_tiny":
        model_student = LDFBNN(num_classes = CLASSES, size = "tiny", ELM_Attention = args.elm_attention, Infor_Recoupling = args.infor_recoupling)
    elif args.model == "ldfbnn_small":
        model_student = LDFBNN(num_classes = CLASSES, size = "small", ELM_Attention = args.elm_attention, Infor_Recoupling = args.infor_recoupling)
    elif args.model == "ldfbnn_middle":
        model_student = LDFBNN(num_classes = CLASSES, size = "middle", ELM_Attention = args.elm_attention, Infor_Recoupling = args.infor_recoupling)
    elif args.model == "ldfbnn_large":
        model_student = LDFBNN(num_classes = CLASSES, size = "large", ELM_Attention = args.elm_attention, Infor_Recoupling = args.infor_recoupling)
    elif args.model == "ldfbnn18":
        model_student = LDFBNN18(num_classes = CLASSES)
    else:
    ...
    if args.baseline == True:
        ckpt = torch.load(args.baseline_model)
        # weights_dict = {}
        # for k, v in ckpt['state_dict'].items():
        #     new_k = k.replace('module.', '') if 'module' in k else k
        #     weights_dict[new_k] = v
        # model_student.load_state_dict(weights_dict, strict=True) # 单张卡加载时

        # model_student.load_state_dict(ckpt['state_dict'], strict=False) # 多张卡加载时
        # f = open('01-model.txt','w')
        # for i,j in ckpt.items():
        #     print(i,j.shape,file = f)
        # f.close()
        model_student.load_state_dict(ckpt, strict=False) # 载入官方权重pth文件时
    ...
    if args.dataset == "ImageNet":
        ...
        if args.dataset == "CIFAR100":
            ...
            # val_dataset = datasets.CIFAR10(root = "datasets/CIFAR10/", train = False, download = False, transform = val_transforms)
    ...
    if args.evaluate:
        if args.local_rank == 0:
            valid_obj, valid_top1_acc, valid_top5_acc = validate(-1, val_loader, model_student, criterion, args)
        # quit()
    ...
    while epoch < args.epochs:
        ...
        if not args.multiprocessing_distributed or args.local_rank == 0:
            ...
            logging.info("Best Acc:{}%, Temp: {}".format(best_top1_acc, temperature))
        ...
        if not args.multiprocessing_distributed or args.local_rank == 0:
            save_checkpoint({
                ...
                'alpha': alpha,
                }, is_best, args.save, epoch = epoch)

        epoch += 1
        
        temperature = adjust_temperature(model_student, epoch).item()
        training_temperature.append(temperature)
...
def train(epoch, train_loader, model_student, model_teacher, criterion, optimizer, scheduler, temperature, args):
    ...
    print_freq = len(train_loader) // 10 # 总共记录10次
        ...
        if args.local_rank == 0:
            if cnt % print_freq == 0:
                ...
                logging.info('Epoch: [{0}][{1}/{2}]\t'
                        ...
                        epoch, cnt, len(train_loader),
                        args.world_size*args.batch_size/batch_time.val,
                        args.world_size*args.batch_size/batch_time.avg,
                        batch_time=batch_time, lr = cur_lr,
                        loss=losses_all, top1=top1, top5=top5))

        if(TRAIN_ONCE and cnt >= 100):
            break
